  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (rsq < cutsq[itype][jtype]) {
        ...
        fpair = frc[tbi][typ2pair[itype][jtype]]*factor_lj/rsq;
...
------------------------------------------------------------------------- */
// pair_coeff * * <sqex_file> <typs> <neutron_bs> nbin_r r_max nbin_q qmax factor update_interval output_interval sq_out gr_out
void PairRMDF::coeff(int narg, char **arg)
{
  ...
  ddr=r_max/nbin_r;
  factor = utils::numeric(FLERR,arg[iarg+4],false,lmp);
  update_interval=utils::numeric(FLERR,arg[iarg+5],false,lmp);
  output_interval=utils::numeric(FLERR,arg[iarg+6],false,lmp);
  ...
  memory->create(dsicqr_dr,nbin_q,nbin_r,"rmdf:dsinqr_dr");
...
void PairRMDF::init_norm()
{
  ...
  for (int qk=0;qk<nbin_q;qk++){
    ...
    for(int rk=0;rk<nbin_r;rk++){
      ...
      dsicqr_dr[qk][rk]=qo4p*(cos(sffa)*sffa-sin(sffa))/sffa/sffa;
...
void PairRMDF::generateForceTable()
{
  ...
  for(int qk=0;qk<nbin_q;qk++){
    temp=(ssq[qk][1]-sqex[qk][0])*wt[qk][0];
    temp1=(ssq[qk][2]-sqex[qk][1])*wt[qk][1];
    eng_vdwl+=temp*(ssq[qk][1]-sqex[qk][0])+temp1*(ssq[qk][2]-sqex[qk][1]);
    for(int i=0;i<npair;i++){
      temp2=temp1*sffn_w[i];
      for(int rk=0;rk<nbin_r;rk++){
        frc[rk][i]-=(temp*sff_w[qk][i]+temp2)*dsicqr_dr[qk][rk];
      }
    }
  }
  eng_vdwl*=(atom->nlocal/2.0);
}