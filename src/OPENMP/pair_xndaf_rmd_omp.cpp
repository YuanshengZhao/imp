...
PairXNDAFRMDOMP::PairXNDAFRMDOMP(LAMMPS *lmp) : PairXNDAFOMP(lmp) {}

/* ---------------------------------------------------------------------- */
// file format : q sqx wtx kx mx sqn wtn kn mn
void PairXNDAFRMDOMP::read_file(char *file)
{
  ...
  if (comm->me == 0) {
    ...
    utils::logmesg(PairXNDAF::lmp,"Reading {} rows\n",nbin_q);
    ...
    utils::logmesg(PairXNDAF::lmp,"Exp S(Q) final data {} {}\n",sqex[nbin_q-1][0],sqex[nbin_q-1][1]);
...
void PairXNDAFRMDOMP::generateForceTable()
{  
  #ifdef XNDAF_DEBUG
  ...
  double diffx,diffn;
#if defined(_OPENMP)
#pragma omp parallel for private(diffx,diffn) reduction( + : localerg )
#endif
