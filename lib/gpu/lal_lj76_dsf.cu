...
__kernel void k_lj76_dsf(const __global numtyp4 *restrict x_,
    ...
    for ( ; nbor<nbor_end; nbor+=n_stride) {
      ...
      if (rsq<lj1[mtype].w) {
        ...
        if (rsq < lj1[mtype].z) {
          ...
          force_lj = factor_lj*r6inv*(lj1[mtype].x/r-lj1[mtype].y);
        } else
          force_lj = (numtyp)0.0;
        ...
        if (EVFLAG && eflag) {
          ...
          if (rsq < lj1[mtype].z) {
            numtyp e=r6inv*(lj3[mtype].x/r-lj3[mtype].y);
...
__kernel void k_lj76_dsf_fast(const __global numtyp4 *restrict x_,
